/**
 * AZ:
 * 2015.12.18 - changed how links are generated, through a configurable callback method
 */
...
(function (root, factory) {
    ...
    function _format(data, options, parentKey) {
        ...
        var hyperlinksEnabled, aTarget, hyperlinkKeys, hyperlinkCback;
        ...
        case OBJECT:
            ...
            hyperlinkCback = options.hyperlinks.cback;
            ...
            for (key in data) {
                ...
                if (typeof(value) === 'string') {
                	var hlEnabled = (hyperlinksEnabled && indexOf.call(hyperlinkKeys, key) >= 0);
                    valNode = scn("td", OBJ_VAL_CLASS_NAME, hyperlinkCback(valNode, value, aTarget, hlEnabled));
                } else {
        ...
        case ARRAY:
            if (data.length > 0) {
                ...
                for (key = 0, len = data.length; key < len; key += 1) {
                    ...
                    if (typeof(value) === "string") {
                        valNode = _format(value, options, key);
                        valNode = scn("td", ARRAY_VAL_CLASS_NAME, hyperlinkCback(valNode, value, aTarget, hyperlinksEnabled));
                    } else {
    ...
    function validateHyperlinkOptions(options){
        var hyperlinks = {
            ...
            keys : [],
            target : '',
            cback: function(child, value, target, enabled){
            	if (enabled) {
        ...
        if (options.hyperlinks) {
        	if (typeof options.hyperlinks == 'function') {
        		hyperlinks.enable = true;
        		hyperlinks.cback = options.hyperlinks;
        		
        	} else if (options.hyperlinks.enable) {
        		hyperlinks.cback = options.hyperlinks.cback || hyperlinks.cback;
                isArray(options.hyperlinks.keys) && (hyperlinks.keys = options.hyperlinks.keys);
                hyperlinks.enable = hyperlinks.keys.length ? true : false;
